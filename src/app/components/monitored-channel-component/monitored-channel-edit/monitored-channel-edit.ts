...
@Component({
  selector: 'app-monitored-channel-edit',
  imports: [MatInputModule, MatButtonModule, MatIconModule, MatSelectModule, ReactiveFormsModule, CommonModule],
  templateUrl: './monitored-channel-edit.html',
  styleUrl: './monitored-channel-edit.css',
})
export class MonitoredChannelEdit implements OnInit {
  ...
  submit(): void {
    if (this.form.valid) {
      ...
      this.cS.update(this.obj).subscribe({ next: () => { this.router.navigate(['/monitored-channels/list']); } });
    }
  }
}
